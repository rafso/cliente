...
if ($_GET["action"] == 'create')
{
	...
	// Third party
	print '<tr><td class="fieldrequired">'.$langs->trans('NomeClienteW').'</td><td colspan="2">'.$soc->getNomUrl(1);
...
//	print '</td></tr>';
	...
	// Tipo obra
	print '<tr><td class="fieldrequired">'.$langs->trans('TipoW').'</td><td colspan="2">'.$propal->tipo_obr;
	$texto = utf8_encode( 'Revestimentos de paredes ou fachadas (Granito; Mármore; Porcelanato; Cerâmica; etc...)');
	print '<input type="checkbox" name="tipo_obr[]" value="'. $texto .'"  /> '. $texto .'<br/>';
	$texto = utf8_encode( 'Pisos (Concreto Polido; Granito; Mármore; Porcelanato; Cerâmica; etc...)');
	print '<input type="checkbox" name="tipo_obr[]" value="'. $texto .'"  /> '. $texto .'<br/>';
	$texto = utf8_encode( 'Tratamento e limpeza de fachadas (Hidrojato; Vedação de trincas; Pintura)');
	print '<input type="checkbox" name="tipo_obr[]" value="'. $texto .'"  /> '. $texto .'<br/>';
	print '<input type="checkbox" name="tipo_obr[]" value="Pinturas; Texturas; Texturatos ou Grafiatos"  /> Pinturas; Texturas; Texturatos ou Grafiatos<br/>';
	print '<input type="checkbox" name="tipo_obr[]" value="Guaritas; Portarias; Entradas"  /> Guaritas; Portarias; Entradas<br/>';
	$texto = utf8_encode( 'Serralheria; Portões; Grades; Guarda Corpo; Corrimão');
	print '<input type="checkbox" name="tipo_obr[]" value="'. $texto .'"  /> '. $texto .'<br/>';
	$texto = utf8_encode( 'Telhados; Impermeabilizações');
	print '<input type="checkbox" name="tipo_obr[]" value="'. $texto .'"  /> '. $texto .'<br/>';
	$texto = utf8_encode( 'Rede Hidráulica');
	print '<input type="checkbox" name="tipo_obr[]" value="'. $texto .'"  /> '. $texto .'<br/>';
	$texto = utf8_encode( 'Rede Elétrica');
	print '<input type="checkbox" name="tipo_obr[]" value="'. $texto .'"  /> '. $texto .'<br/>';
	print '<input type="checkbox" value="Outros:"  /> Outros <input type="text" size="100" maxlength="200" name="tipo_obr[]" id="tipo_outros" value=""> <br/>';
	print '</td>';
	print '</tr>';
...
//	print '</td></tr>';
	...
	// Date
	print '<tr><td class="fieldrequired">'.$langs->trans('DateSOW').'</td><td colspan="2">';
	$html->select_date('','','','','',"addprop");
	print '</td></tr>';
	
	// Data da visita
	print '<tr><td class="fieldrequired">'.$langs->trans('DateVisW').'</td><td colspan="2">';
	//$set_time='', $prefix='re', $h=0, $m=0, $empty=0, $form_name="", $d=1, $addnowbutton=0, $nooutput=0, $disabled=0, $fullday=''
	$html->select_date('','data_hora_visita',1,1,1,'addprop');
	//$html->select_date('','','','','',"addprop");
	print '</td></tr>';
...
//    print '</td></tr>';
	...
	print '<td colspan="2">';
	$html->select_date('','data_envio',0,0,1,'addprop');
	print '</td></tr>';

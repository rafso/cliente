	...
	if ($action != 'presend')
	{
		...
		if ($action != 'statut' && $action <> 'editline')
		{
			...
			if ($object->statut == 0 && $user->rights->propale->valider)
			{
				...
				//removido para habilitar o botão de confirmar orçamento mesmo sem nenhum item, era:
...
else
{
	...
	//kurt verificar se é para exibir as visitas de hoje e vencidas, vencidas verificar se a data é < que hoje e se o estado <> concluída (está em baixo a outra validação)
	//fazer um select separado para listar as visitas atrasadas, algumas tem cliente associado e outras não
	if ($past <> ''){
		...
		$sql .= ' AND ac.percent <> 100'; //não concluída
		...
		$sql .= ' AND ac.percent <> 100'; //não concluída
		$sql .= ' ORDER BY ac.datep ASC';
	} else { //se não for para ver os atrasados, e nem proximos filtra normal da tela
	...
	if ($result)
	{
		...
		if (($past <>'') || ($next <> '')){ //se estiver mostrando as visitas atrasadas, não exibe as opções de filtro
...
//			print '<td>'.$langs->trans('Status').'</td>';
		...
		//kurt alterado de preço para data da visita, mais util no cabeçalho
...
//				print '</td>';
			...
			} else { //kurt, se estiver exibindo os atrasados, não exibe o link padrão para o orçamento
			...
			print '<td align="left">';
			if ($objp->rowid){ //kurt se estiver associada a um cliente ou fornecedor, exibe o nome, caso contrário exibe uma mensagem padrão no lugar do nome
				print $companystatic->getNomUrl(1,'customer');
			} else { //exibe a descrição da ação na agenda no lugar do nome
				$url = DOL_URL_ROOT. '/comm/action/fiche.php?id=' . $objp->idfiche;
			...
			//kurt, essa parte não é necessária se estiver mostrando as atrasadas ou proximas ações
			if (($past <> '') || ($next <> '')){
			} else {
				// Date propale

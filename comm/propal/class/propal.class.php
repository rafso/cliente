...
class Propal extends CommonObject
{
	...
	function Propal($DB, $socid="", $propalid=0)
	{
		...
		//wallner eng
		if (!empty($_POST['tipo_obr'])){
			$this->tipo_obra = implode(',', $_POST['tipo_obr'] );
		} else {

		}
		$this->data_hora_visita = $_POST['data_hora_visita'] ." ". $_POST['data_hora_visitahour'] .":". $_POST['data_hora_visitamin'];
		$this->data_envio = $_POST['data_envio'];
		...
		$this->labelstatut[5]=($langs->trans("VisitasDia"));
		...
		$this->labelstatut_short[5]=($langs->trans("VisitasDia"));
	}
	...
	function create($user='', $notrigger=0)
	{
		...
		if (empty($this->tipo_obra)){
			$this->tipo_obra = '';
		}
		if (empty($this->data_hora_visita)){
			$this->data_hora_visita = '';
		}
		if (empty($this->data_envio)){
			$this->data_envio = '';
		}
		$date_hora = explode(" ",$this->data_hora_visita);
		$data = explode("/", $date_hora[0]);
		
		$ano = $data[2];
		$mes = $data[1];
		$dia = $data[0];
		
		$novo = $ano ."/". $mes ."/". $dia ." ". $date_hora[1] .":00";
		
		$data_envio = explode("/", $this->data_envio);
		...
		$sql.= ",'' "; //era $this->cond_reglement_id
		$sql.= ",'' "; //era $this->mode_reglement_id;
		...
		if ($resql)
		{
			...
			if (! $error)
			{
				...
				//KURT se inseriu sem erro, manda agora inserir um evento na agenda com a data da visita, para o kurt	
				//inserido para integração com o calendário do dollibar, insere um registro na tabela do calendário
				$titulo = "Visitar: " . $this->client->nom . " tel: " . $this->client->tel;
				$end = $this->client->address;
				$hoje = date("Y/m/d H:i:s");
				
				$query = "INSERT INTO `llx_actioncomm` (`datep`,`datep2`,`fk_action`,`label`,`datec`,`fk_user_author`,`fk_user_action`,`priority`,`percent`,`location`, fk_soc)";
				$query .= " VALUES ('". $novo. "',";
				$query .= "'". $novo ."',";
				$query .= "52,";
				$query .= "'". $titulo ."',";
	...
	function fetch($rowid,$ref='')
	{
		...
		$sql.= ", cp.code as mode_reglement_code, cp.libelle as mode_reglement, p.tipo_obr, p.data_hora_visita, p.data_envio";
		...
		if ($resql)
		{
			if ($this->db->num_rows($resql))
			{
				...
				//novos campos wallner eng
				$this->tipo_obr				= $obj->tipo_obr;
				$this->data_hora_visita		= $obj->data_hora_visita;
	...
	//seta a data e hora da visita
	function set_date2($user, $date)
	{
		if ($user->rights->propale->creer)
		{
			$sql = "UPDATE ".MAIN_DB_PREFIX."propal SET data_hora_visita = '". $date . "'";
			$sql.= " WHERE rowid = ".$this->id." AND fk_statut = 0";
			if ($this->db->query($sql) )
			{
				$this->data_hora_visita = $date;
				return 1;
			}
			else
			{
	...
	//seta a data do envio
	function set_date3($user, $date)
	{
		if ($user->rights->propale->creer)
		{
			$sql = "UPDATE ".MAIN_DB_PREFIX."propal SET data_envio = '". $date . "'";
			$sql.= " WHERE rowid = ".$this->id." AND fk_statut = 0";
			if ($this->db->query($sql) )
			{
				$this->dat_envio = $date;
...
/**
     ...
     *      @param          mode    "opened" pour propal a fermer, "signed" pour propale a facturer, next para visitas do dia seguinte
     *      @return         int     <0 if KO, >0 if OK
	 */
	function load_board($user,$mode)
	{
		...
		//verificação do atraso de visita, teve que ser query separada porque a de baixo dessa roda apenas pelos registros encontrados e como já estou filtrando em cima somente pelos datadif = 1, não funciona, tenho que buscar agora os datadiff <= 0 para saber os atrasados e do dia
		if ($mode == 'next')
		{
			//lista os itens da agenda do tipo visita (criado manualmente) e visita requisitada pelo site, que não estão completadas e estão atrasadas
			$sql = "SELECT id";
			$sql.= " FROM ".MAIN_DB_PREFIX."actioncomm as ac";
			$sql .= " WHERE (ac.fk_action = 51 OR ac.fk_action = 52) AND (DATEDIFF(ac.datep , NOW()) <=0 ) AND ac.percent <> 100";
			$resql=$this->db->query($sql);
			while ($obj=$this->db->fetch_object($resql))  //enquanto tiver registro é atrasado, o ideal seria apenas um count
			{
				$this->nbtodolate++;
			}
			
			//faz a verificação das visitas de amanhã
			$clause = " WHERE";
			
			$sql = "SELECT id ";
			$sql.= " FROM ".MAIN_DB_PREFIX."actioncomm as ac";
			$sql.= $clause." (ac.fk_action = 51 OR ac.fk_action = 52) AND (DATEDIFF(ac.datep , NOW()) = 1 ) AND ac.percent <> 100 ";
			...
			$sql = "SELECT p.rowid, p.ref, p.datec as datec, p.fin_validite as datefin, DATE(data_hora_visita) as data_visita ";
			...
			if ($mode == 'next') $sql.= " AND (DATEDIFF(`data_hora_visita` , NOW()) = 1)";
		...
		if ($resql)
		{
			...
			if ($mode == 'next') $delay_warning=86400; //minutos de um dia kurt

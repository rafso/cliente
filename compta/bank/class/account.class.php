...
class Account extends CommonObject
{
	...
	//função para retornar o select de centro de custo e subcentro de custo
	//recebe 1 para centro de custo e 2 para subcentro de custo e o selected recebe o texto a ser selecionado se for o caso
	//$controle faz o gerenciamento se é os campos da busca ou do cadastro para colocar com nomes diferentes no select
	function printCentro($selected = '', $subcentro = '', $controle = ''){
		?>
		<script type="text/javascript">
		function exibeModeloSelect(id_centro, id_sub, controle) {
		$.ajax(
		{
		type: "POST",
		url: "../../includes/util_kurt.php",
		data: "action="+ controle + "&id_centro=" + id_centro + "&id_sub = " + id_sub,
		beforeSend: function() {
...
////			
//			$sql1 = 'SELECT id, descricao FROM llx_sub_centro_custo WHERE fk_id_centro = 1';
//			$resql1 = $this->db->query($sql1);
////			$select1 .= '<select name="sub-centro-custo">';	
////			if ($resql1) {
//				while ($obj1=$this->db->fetch_object($resql1)) {
//						$id_sub .= ";" . $obj1->id;
//						$desc_sub .= ";" . $obj1->descricao;	
//					}
////				}
////			}
////			$select1 .= '</select>';
		...
		if ($controle == 1) {
			$select .= '<select name="centro" id="centro" onChange="exibeModeloSelect(this.value, \''.$subcentro.'\', \''. $controle .'\')"><option value="0"></option>';
		} else {
			$select .= '<select name="centroSearch" id="centroSearch" onChange="exibeModeloSelect(this.value, \''.$subcentro.'\', \''. $controle .'\')"><option value="0"></option>';
		}
			$sql = 'SELECT id, descricao FROM llx_centro_custo';
			$resql = $this->db->query($sql);			
			if ($resql) {
				while ($obj=$this->db->fetch_object($resql)) {
					if (strstr($selected, $obj->id)){
						$select .= '<option value="'.$obj->id.'" selected>'.$obj->descricao.'</option>';
						} else {
							$select .= '<option value="'.$obj->id.'">'.$obj->descricao.'</option>';	
							}
				}
			}
	...
	function printSubCentro($id){
		switch ($id) {
			case "1":
				echo utf8_encode('Fichados');
				break;
			case '2':
				echo utf8_encode('Medição');
				break;
			case '3':
				echo utf8_encode('Acabamentos');
				break;
			case '4':
				echo utf8_encode('Material básico');
				break;		
			case '5':
				echo utf8_encode('Material elétrico');
				break;		
			case '6':
				echo utf8_encode('Material hidráulico');
				break;	
			case '7':
				echo utf8_encode('Esquadrias');
				break;	
			case '8':
				echo utf8_encode('Impermeabialização');
				break;	
			case '9':
				echo utf8_encode('Aquisição');
				break;		
			case '10':
				echo utf8_encode('Locação');
				break;	
			case '11':
				echo utf8_encode('Manutenção');
				break;																														
		}
	}
     ...
     */
	function addline($date, $oper, $label, $amount, $num_chq='', $categorie='', $user, $emetteur='',$banque='', $centro='', $subcentro='')
    {
        // Clean parameters
        $emetteur=trim($emetteur);
        $banque=trim($banque);
		$centro=trim($centro);
		$subcentro=trim($subcentro);
		...
		//kurt
		$sql.= ", ".($centro?"'". $centro ."'":"null");
		$sql.= ", ".($subcentro?"'". $subcentro ."'":"null");
        $sql.= ")";
...
class AccountLine extends CommonObject
{
    ...
    function fetch($rowid,$ref='',$num='')
    {
        ...
        $sql.= " b.fk_bordereau, b.banque, b.emetteur, b.origem, b.subcentro,";
    ...
    function update($user,$notrigger=0)
    {
		...
		$sql.= " origem='".$this->db->idate($this->centro)."'";
		$sql.= " subcentro='".$this->db->idate($this->subcentro)."'";
        $sql.= " WHERE rowid = ".$this->rowid;

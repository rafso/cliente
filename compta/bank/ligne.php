...
if ($action == "update")
{
	...
	if (isset($_POST['centro']))   $sql.=" origem='".$db->escape($_POST["centro"])."',";
	if (isset($_POST['subcentro1']))   $sql.=" subcentro='".$db->escape($_POST["subcentro1"])."',";
...
$sql.= " b.emetteur,b.banque, b.origem, b.subcentro, c.descricao, sc.descricao";
$sql.= " FROM ".MAIN_DB_PREFIX."bank as b";
$sql .= ' LEFT JOIN llx_centro_custo as c ON b.origem = c.id';
$sql .= ' LEFT JOIN llx_sub_centro_custo as sc ON b.subcentro = sc.id';
$sql.= " WHERE rowid=".$rowid;
...
if ($result)
{
    ...
    if ($db->num_rows($result))
    {
		...
		// centro de custo
		print "<tr><td>".$langs->trans("origem")."</td>";
		print '<td colspan="3">';
		print $acct->printCentro($objp->origem, $objp->subcentro, 1);
		//print '<input name="origem" maxlength="190" class="flat" value="'. $objp->origem .'"';
		print '</td>';
		print '</tr>';
				
		//subcentro de custo		
		print "<tr><td>".$langs->trans("sub-centro")."</td>";
		$_REQUEST['lignecentro'] = $objp->origem;
		$_REQUEST['lignesubcentro'] = $objp->subcentro;

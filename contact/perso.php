...
if ($_GET["action"] == 'edit')
{
	...
	// Name
    print '<tr><td width="20%">'.$langs->trans("Label").'</td><td width="30%">'.$contact->nom.'</td>';
	print '<td width="20%">'.$langs->trans("Company").'</td><td width="30%">'.$contact->prenom.'</td>';
    ...
    if (empty($conf->global->SOCIETE_DISABLE_CONTACTS))
    {
        if ($contact->socid > 0)
        {
			...
			print '<tr><td>'.$langs->trans("empreW").'</td><td colspan="3">'.$objsoc->getNomUrl(1).'</td>';
        }
        else
        {
...
else
{
	...
	// Name
    print '<tr><td width="20%">'.$langs->trans("Label").'</td><td width="30%">'.$contact->name.'</td>';
	print '<td width="20%">'.$langs->trans("Company").'</td><td width="30%">'.$contact->firstname.'</td></tr>';
    ...
    if (empty($conf->global->SOCIETE_DISABLE_CONTACTS))
    {
        if ($contact->socid > 0)
        {
			...
			print '<tr><td>'.$langs->trans("empreW").'</td><td colspan="3">'.$objsoc->getNomUrl(1).'</td></tr>';
        }

...
$sql = "SELECT s.rowid as socid, s.nom, s.tipo_fornecedor, s.idprof4, s.datec, s.datea,  st.libelle as stcomm, s.prefix_comm, s.status as status, ";
...
if ($search_ville) $sql .= " AND s.tipo_fornecedor LIKE '%".$db->escape($search_ville)."%'";
if ($search_code_fournisseur)   $sql .= " AND s.code_fournisseur LIKE '%".$db->escape($search_code_fournisseur)."%'";
if ($search_compta_fournisseur) $sql .= " AND s.idprof4 LIKE '%".$db->escape($search_compta_fournisseur)."%'";
...
if ($resql)
{
	...
	print '<tr class="liste_titre">';
	print_liste_field_titre($langs->trans("CompanyName"),$_SERVER["PHP_SELF"],"s.nom","",$param,'valign="middle"',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans("nomeEmpW"),$_SERVER["PHP_SELF"],"s.idprof4","",$param,'valign="middle"',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans("TipoFW"),$_SERVER["PHP_SELF"],"s.tipo_fornecedor","",$param,'align="left"',$sortfield,$sortorder);
	...
	print '<td class="liste_titre"><input type="text" class="flat" name="search_compta_fournisseur" value="'.$search_compta_fournisseur.'"></td>';

	print '<td align="left" class="liste_titre">';
	print '<input class="flat" type="text" size="10" name="search_ville" value="'.$search_ville.'">';
	print '</td>';
...
//	print '</td>';
	...
	while ($i < min($num,$conf->liste_limit))
	{
		...
		print '<td align="left">'.$obj->tipo_fornecedor.'&nbsp;</td>';

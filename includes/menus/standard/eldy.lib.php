...
function print_eldy_menu($db,$atarget,$type_user)
{
	...
	if ($conf->product->enabled || $conf->service->enabled)
	{
		...
		if ($user->rights->produit->lire || $user->rights->service->lire)
		{
			...
			print '<div class="'.$id.' '.$idsel.'"><span class="'.$id.' tmenuimage"  id="mainmenuspan_'.$idsel.'"></span></div>';
			print '</a>';
			//era /product/index.php?mainmenu=products&amp;leftmenu=
			///product/liste.php?leftmenu=service&type=1
...
/* comentado para remover o menu novo fornecedor que aparecia no topo de tudo do menu na área de clientes
...
//adicionando novo estado de cliente no menu
				if ($leftmenu=="prospects") $newmenu->add("/comm/prospect/prospects.php?sortfield=s.datec&amp;sortorder=desc&amp;begin=&amp;stcomm=4", $langs->trans("LastProspectContactWait"), 2, $user->rights->societe->lire);
        ...
        if ($mainmenu == 'commercial')
        {
            ...
            if (! empty($conf->propal->enabled))
            {
				...
				if ($leftmenu=="propals") $newmenu->add("/comm/propal.php?leftmenu=propals&next=1", $langs->trans("VisitasDia"), 2, $user->rights->propale->lire);
				if ($leftmenu=="propals") $newmenu->add("/comm/propal.php?leftmenu=propals&past=1", $langs->trans("VisitasPast"), 2, $user->rights->propale->lire);
        ...
        if ($mainmenu == 'products')
        {
            ...
            if ($conf->service->enabled)
            {
				...
				$newmenu->add("/product/liste.php?leftmenu=service&amp;type=1", $langs->trans("ListReq"), 1, $user->rights->service->lire);
//                if ($conf->propal->enabled)
//                {

...
//KURT número de orçamentos/visitas para o dia seguinte
	$langs->load("propal");
	
	include_once(DOL_DOCUMENT_ROOT."/comm/propal/class/propal.class.php");
	$board=new Propal($db);
	$board->load_board($user,"next");
	$board->label=$langs->trans("VisitasDia");
	$board->url=DOL_URL_ROOT.'/comm/propal.php?next=1&mainmenu=commercial&leftmenu=propals'; //para mudar o menu para o da agenda usar: &idmenu=5207&mainmenu=agenda&leftmenu=
	$board->img=img_object($langs->trans("Contract"),"contract");
	$rowspan++;
	$dashboardlines[]=$board;
...
foreach($dashboardlines as $key => $board)
{
	...
	//kurt imprimindo uma URL diferente para o alerta de visitas porque vou exibir um link para a lista de orçamentos em atraso
	if ($board->label == $langs->trans("VisitasDia")){
		print '<a href="'. DOL_URL_ROOT.'/comm/propal.php?past=1&mainmenu=commercial&leftmenu=propals">'; //para mudar o menu para o da agenda usar: &idmenu=5207&mainmenu=agenda&leftmenu=
	} else {

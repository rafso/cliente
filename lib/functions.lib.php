...
function img_action($alt = "default", $numaction)
{
    ...
    if ($alt=="default") {
		...
		if ($numaction == 4)  $alt=$langs->transnoentitiesnoconv("ChangeContactWait");
    }
...
function restrictedArea($user, $features='societe', $objectid=0, $dbtablename='', $feature2='', $dbt_keyfield='fk_soc', $dbt_select='rowid')
{
    ...
    $readok=1;
	$a = $user;
	$b = $user->rights->fournisseur->lire;
    foreach ($features as $feature)
    {
		$a = $user->rights->fournisseur->lire;
		$b = $user->rights->societe->lire;
        if ($feature == 'societe')
        {
    ...
    if ( GETPOST("action") && GETPOST("action")  == 'create')
    {
        foreach ($features as $feature)
        {
			...
			else if($feature == 'societe') //se for societe, pode ser cliente ou fornecedor, se tiver uma delas tem que exibir
			{
				if (! $user->rights->societe->lire && ! $user->rights->fournisseur->lire) 	$createok=0;				
			}

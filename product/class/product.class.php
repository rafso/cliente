...
//	var $stock_warehouse=array();
	...
	function create($user,$notrigger=0)
	{
		...
		$this->data_prevista = trim($this->data_prevista);
		$this->data_emissao = trim($this->data_emissao);
		...
		for ($i = 1; $i <= 50; $i++){
			...
			$this->$qtd = trim($this->$qtd);
			$this->$un = trim($this->$un);
		}
		...
		if ($result)
		{
			...
			{*/
				...
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."materiais (";
				$sql.= "etiqueta";
				$sql.= ", descricao";
				$sql.= ", data_prevista";
				$sql.= ", data_emissao";
				...
				$sql.= " '".$this->etiqueta."'";
				$sql.= ", '".$this->desc."'";
				$sql.= ", '".$this->data_prevista."'";
				$sql.= ", DATE_FORMAT( CURDATE( ) , '%d/%m/%Y' )"; //insere a data atual
				$sql.= ", '".$this->estado."'";
				for ($i = 1; $i <= 50; $i++){
					$item = "item" . $i;
					$qtd = "qtd" . $i;
					$un = "unidade" . $i;
					$sql.= ", '".$this->$item."'";
					$sql.= ", ".($this->$qtd? "'".$this->$qtd."'":"null");
					$sql.= ", ".($this->$un? "'".$this->$un."'":"null");
				}
				$sql.= ")";
...
//		{
//			$this->db->commit();
		return $id = $this->db->last_insert_id(MAIN_DB_PREFIX."materiais");
//		}
//		else
//		{
//			$this->db->rollback();
...
//		$this->accountancy_code_sell= trim($this->accountancy_code_sell);
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."materiais";
		$sql.= " SET etiqueta = '".$this->etiqueta."'";
		$sql.= ", descricao = '".$this->desc."'";
		$sql.= ", data_prevista = '".$this->data_prevista."'";
	...
	function verif_prod_use($id)
	{
		...
		$elements = array('propaldet','commandedet','facturedet','contratdet','product_fournisseur', 'materiais');
...
//				$elements = array('materiais');
//
//				foreach($elements as $table)
//				{
					$sql = "DELETE FROM ".MAIN_DB_PREFIX."materiais";
					$sql.= " WHERE id = ".$id;
					$result = $this->db->query($sql);
//				}
...
//		}
	...
	function fetch($id='',$ref='')
	{
		...
		$sql = "SELECT id, etiqueta, descricao, data_prevista, data_emissao, estado ";
		for ($i = 1; $i <= 50; $i++){
			$item = "item" . $i;
			$qtd = "qtd". $i;
			$un =  "unidade". $i;
		...
		$sql.= " FROM ".MAIN_DB_PREFIX."materiais";
		$sql.= " WHERE id = '".$id."'";
		...
		if ( $resql )
		{
			if ($this->db->num_rows($resql) > 0)
			{
				...
				$this->id				= $object->id;
				$this->etiqueta			= $object->etiqueta;
				$this->desc				= $object->descricao;		// TODO deprecated
				$this->data_prevista	= $object->data_prevista;
				$this->data_emissao		= $object->data_emissao;

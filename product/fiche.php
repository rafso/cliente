...
//	}
	...
	if (! $error)
	{
		$product->etiqueta                = $etiqueta;
		$product->desc					  = $desc;
		$product->data_prevista			  = $data_prevista;
...
if ($action == 'confirm_delete' && $confirm == 'yes')
{
	...
	if ( ($product->type == 0)	|| ($product->type == 1) ) // && $user->rights->produit->supprimer && $user->rights->service->supprimer
	{
	...
	if ($result == 0)
	{
		Header('Location: '.DOL_URL_ROOT.'/product/liste.php?delprod='.$product->id);
		exit;
	}
	else
	{
...
//
//		print '<tr>';
//		print '<td class="fieldrequired" width="20%">'.$langs->trans("Ref").'</td><td><input name="ref" size="40" maxlength="40" value="'.$ref.'">';
//		if ($_error == 1)
//		{
...
//		}
		...
		//cabeçalho da requisição de materiais KURT
		print '<table class="border" width="100%">';
		print '<tr><td class="fieldrequired">'.$langs->trans("Label").'</td><td><input name="etiqueta" size="40" maxlength="40" value="'.$_POST["etiqueta"].'"></td>';
		print '<td class="fieldrequired">'.$langs->trans("dataentre").'</td><td><input name="data_prevista" size="10" maxlength="20" value="'.$_POST["data_prevista"].'"></td></tr>';
		// Description (used in invoice, propal...)
		print '<tr><td valign="top" class="fieldrequired">'.$langs->trans("Description").'</td><td>';
		print '<input name="desc" size="70" maxlength="250" value="'.$_POST["desc"].'"></td>';
		print '<td class="fieldrequired">'.$langs->trans("dataem").'</td><td>'. date("d/m/Y") .'</td></tr>';	
		print '<tr><td class="fieldrequired">'.$langs->trans("estado").'</td><td  colspan="3">';
		print '<select name="estado" id="estado">';
		print '<option>Em aberto';
		print '<option>Atendida';
		print '<option>Cancelada';
		print '</select>';
		print '</td></tr>';
		print '</table>';
		print '<p/>';
		...
		print '<table class="border" width="100%">';
		print '<tr><td valign="top" align="center" class="fieldrequired" bgcolor="EEEEEE">'.$langs->trans("item").'</td>';
		print '<td valign="top" align="center" class="fieldrequired" bgcolor="EEEEEE">'.$langs->trans("discri").'</td>';
		print '<td valign="top" align="center" class="fieldrequired" bgcolor="EEEEEE">'.$langs->trans("uni").'</td>';
		print '<td valign="top" align="center" class="fieldrequired" bgcolor="EEEEEE">'.$langs->trans("Quantity").'</td></tr>';
		
		print '<script language="javascript" type="text/javascript">';
		print 'function imposeMaxLength(Object, MaxLen)';
		print '{';
		print 'return (Object.value.length <= MaxLen);';
		print '}';
		print '</script> ';
...
//		$doleditor->Create();
		//exibe os 50 campos para itens
		for ($i = 1; $i <= 50; $i++){
			print '<tr><td width="3%" align="center">'. $i .'</td><td width="20%"><textarea rows="2" cols="110" name="item'. $i .'" onkeypress="return imposeMaxLength(this, 5000);" value="'.$_POST["item'. $i .'"].'"></textarea></td>';
			print '<td width="5%" align="center"><input name="unidade'. $i .'" size="10" maxlength="10" value="'.$_POST["unidade $i .'"].'"></td><td width="5%" align="center"><input name="qtd'. $i .'" size="10" maxlength="10" value="'.$_POST["qtd'. $i .'"].'"></td></tr>';
		}
...
//	}
	...
	if ($action == 'edit')
	{
		if (empty($usecanvas))
		{
			...
			// campos edição
				...
				print '<td class="fieldrequired">'.$langs->trans("dataentre").'</td><td><input name="data_prevista" size="10" maxlength="20" value="'.$product->data_prevista .'"></td></tr>';
				// Description (used in invoice, propal...)
				print '<tr><td valign="top" class="fieldrequired">'.$langs->trans("Description").'</td><td>';
				print '<input name="desc" size="70" maxlength="250" value="'.$product->desc.'"></td>';
				print '<td class="fieldrequired">'.$langs->trans("dataem").'</td><td>'. $product->data_emissao .'</td></tr>';	
				print '<tr><td class="fieldrequired">'.$langs->trans("estado").'</td><td  colspan="3">';
				print '<select name="estado" id="estado">';
				
				if (strstr($product->estado, "Em aberto")){
					print '<option selected >Em aberto';
				} else print '<option>Em aberto';

				if (strstr($product->estado, "Atendida")){
					print '<option selected >Atendida';
				} else print '<option>Atendida';
				
				if (strstr($product->estado, "Cancelada")){
				...
				for ($i = 1; $i <= 50; $i++){
					...
					$qtd = "qtd" . $i;
					$un = "unidade" . $i;
					print '<tr><td width="3%" align="center">'. $i .'</td><td width="20%"><textarea rows="2" cols="110" name="item'. $i .'" onkeypress="return imposeMaxLength(this, 5000);" >'.$product->$item.'</textarea></td>';	
					print '<td width="5%" align="center"><input name="unidade'. $i .'" size="10" maxlength="10" value="'.$product->$un.'"></td><td width="5%" align="center"><input name="qtd'. $i .'" size="10" maxlength="10" value="'.$product->$qtd.'"></td></tr>';
				}
	...
	else
	{
		...
		if ($action == 'delete')
		{
			$formconfirm=$html->formconfirm($_SERVER["PHP_SELF"].'?id='.$product->id, $langs->trans('DeleteProduct'), $langs->trans('ConfirmDeleteProduct'), 'confirm_delete','',0,1);
			print $formconfirm;
		}
...
//            $isphoto=$product->is_photo_available($conf->product->dir_output);
			...
			print '<table class="border" width="100%"><tr>';
				print '<tr><td class="fieldrequired">'.$langs->trans("Label").'</td><td>'.$product->etiqueta.'</td>';
				print '<td class="fieldrequired">'.$langs->trans("dataentre").'</td><td>'.$product->data_prevista .'</td></tr>';
				// Description (used in invoice, propal...)
				print '<tr><td valign="top" class="fieldrequired">'.$langs->trans("Description").'</td><td>';
				print $product->desc;
				print '<td class="fieldrequired">'.$langs->trans("dataem").'</td><td>'. $product->data_emissao .'</td></tr>';	
				print '<tr><td class="fieldrequired">'.$langs->trans("estado").'</td><td  colspan="3">';
				print $product->estado;
				print '</td></tr>';
				print '</table>';
				print '<p/>';
				...
				for ($i = 1; $i <= 50; $i++){
					...
					$qtd = "qtd" . $i;
					$un = "unidade" . $i;
					print '<tr><td width="3%" align="center">'. $i .'</td><td width="20%">'.$product->$item.'</td>';
					print '<td width="5%" align="center">'.$product->$un.'</td><td width="5%" align="center">'.$product->$qtd.'</td></tr>';
				}	

//			print '<td width="10%">'.$langs->trans("Label").'</td><td colspan="3">'. $product->etiqueta .'</td></tr>';
//			print '<tr><td width="10%">'.$langs->trans("Description").'</td><td colspan="3">'. $product->desc .'</td></tr>';
//			for ($i = 1; $i <= 50; $i++){
//				$item = "item" . $i;
//				$qtd = "qtd". $i;
//				print '<tr><td width="10%">'.$langs->trans("item"). " " . $i .'</td><td width="40%">'. $product->$item .'</td>';
//				print '<td width="10%">'.$langs->trans("Quantity").'</td><td width="40%">'. $product->$qtd .'</td></tr>';
//			}

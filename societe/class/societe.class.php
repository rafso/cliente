...
class Societe extends CommonObject
{
    ...
    function create($user='')
    {
		...
		//wallner eng
		if (!empty($_POST['como_con'])){
			$this->como_conheceu = implode(',', $_POST['como_con'] );
		} else {
			
		}
		if (!empty($_POST['tipo_fornecedor'])){
			$this->tipo_fornecedor = $_POST['tipo_fornecedor'];
		} else {
        ...
        if ($result >= 0)
        {
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."societe (nom, entity, datec, datea, fk_user_creat, canvas, status, ref_int, rua, numero, complemento, bairro, como_conheceu, tipo_fornecedor, cp, cpf_cnpj, email_2, ponto_ref, tel1_soli, tel2_soli)";
			...
			$sql.= ", '".$this->bairro . "'";
			$sql.= ", '".$this->como_conheceu . "'";
			$sql.= ", '".$this->tipo_fornecedor . "'";
			$sql.= ", '".$this->cp . "'";
			$sql.= ", '".$this->cpf_cnpj . "'";
			$sql.= ", '".$this->email_2 . "'";
			$sql.= ", '".$this->ponto_ref . "'";
    ...
    function update($id, $user='', $call_trigger=1, $allowmodcodeclient=0, $allowmodcodefournisseur=0)
    {
		...
		$this->como_conheceu        = trim($this->como_conheceu);
		$this->tipo_fornecedor        = trim($this->tipo_fornecedor);
		$this->cpf_cnpj				= trim($this->cpf_cnpj);
		$this->email_2				= trim($this->email_2);
		$this->ponto_ref			= trim($this->ponto_ref);
		$this->tel1_soli			= trim($this->tel1_soli);
		$this->tel2_soli			= trim($this->tel2_soli);
        ...
        if ($result >= 0)
        {
			...
			//campos novos
			$sql .= ",rua = ".($this->rua?"'".$this->db->escape($this->rua)."'":"null");
			$sql .= ",numero = ".($this->numero?"'".$this->db->escape($this->numero)."'":"null");
			$sql .= ",complemento = ".($this->complemento?"'".$this->db->escape($this->complemento)."'":"null");
			$sql .= ",bairro = ".($this->bairro?"'".$this->db->escape($this->bairro)."'":"null");
			//wallner eng
			if (!empty($_POST['como_con'])){
				$this->como_conheceu = implode(',', $_POST['como_con'] );
			} else {
			...
			$sql .= ",como_conheceu = ".($this->como_conheceu?"'".$this->db->escape($this->como_conheceu)."'":"null");
			$sql .= ",tipo_fornecedor = ".($this->tipo_fornecedor?"'".$this->db->escape($this->tipo_fornecedor)."'":"null");
			$sql .= ",cpf_cnpj = ".($this->cpf_cnpj?"'".$this->db->escape($this->cpf_cnpj)."'":"null");
			
			$sql .= ",email_2 = ".($this->email_2?"'".$this->db->escape($this->email_2)."'":"null");
			$sql .= ",ponto_ref = ".($this->ponto_ref?"'".$this->db->escape($this->ponto_ref)."'":"null");
			$sql .= ",tel1_soli = ".($this->tel1_soli?"'".$this->db->escape($this->tel1_soli)."'":"null");
			$sql .= ",tel2_soli = ".($this->tel2_soli?"'".$this->db->escape($this->tel2_soli)."'":"null");
    ...
    function fetch($rowid, $ref='', $ref_ext='', $ref_int='', $idprof1='',$idprof2='',$idprof3='',$idprof4='')
    {
        ...
        $sql .= ', s.tel, s.fax, s.email, s.url, s.cp, s.ville as town, s.note, s.client, s.fournisseur';
        ...
        //novos campos
		$sql .= ', s.rua, s.numero, s.complemento, s.bairro, s.como_conheceu, s.tipo_fornecedor, s.cpf_cnpj, s.tel2_soli, s.tel1_soli, s.ponto_ref, s.email_2';
		//fim novos campos
		$sql .= ', s.import_key';
        ...
        if ($resql)
        {
            ...
            if ($num)
            {
				...
				$this->como_conheceu        = $obj->como_conheceu;
				$this->tipo_fornecedor      = $obj->tipo_fornecedor;
				$this->cpf_cnpj				= $obj->cpf_cnpj;
				$this->email_2				= $obj->email_2;
				$this->ponto_ref			= $obj->ponto_ref;
				$this->tel1_soli			= $obj->tel1_soli;
				$this->tel2_soli			= $obj->tel2_soli;
